...
def get_dataset_list(dataset_path):
    ...
    script_file_names, scripts, normalized_scripts = get_metadata(metadata_path)
...
def get_metadata(metadata_path):
    ...
    with open(metadata_path) as metadata:
        ...
        lines = ""
        for line in reader:
            lines = lines + "".join(line).replace("\n", "")
        lines = lines.split("LJ")
        lines.pop(0)
        for line in lines:
            full_txt = line.split("|")
            script_file_name = "LJ" + full_txt[0]
            script_file_names.append(script_file_name)
            scripts[script_file_name] = full_txt[1].lower()
            normalized_scripts[script_file_name] = sp.script_normalize(full_txt[2])
...
def get_idx2char(dataset_list):
    ...
    with open("./dataset/ljspeech_vocab.json", "w") as f:
